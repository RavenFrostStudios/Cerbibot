...
class XAIAdapter(ProviderAdapter):
    """xAI/Grok provider via OpenAI-compatible API surface."""

    def __init__(self, config: ProviderConfig, rate_limiter=None):
        super().__init__(provider_name="xai", config=config, rate_limiter=rate_limiter)
        ...
        try:
            ...
            async for chunk in stream:
                if time.monotonic() - start > timeout_seconds:
                    raise ProviderTimeoutError(f"xAI stream timed out after {timeout_seconds}s")
                delta = chunk.choices[0].delta if chunk.choices else None
                text = getattr(delta, "content", None)
                if text:
                    yield text
        except APIConnectionError as err:
            raise RuntimeError(f"xAI streaming connection failed: {err}") from err
    ...
    async def _complete(
        ...
        for i in range(attempts):
            try:
                ...
                if not is_retryable_error(
                    ...
                    raise RuntimeError(
                        f"xAI completion failed with non-retryable error"
        ...
        if isinstance(last_error, asyncio.TimeoutError):
            raise ProviderTimeoutError(
                f"xAI call timed out after {attempts} attempts (timeout={timeout_seconds}s)"
            ) from last_error
        raise RuntimeError(f"xAI completion failed after retries: {last_error}")
